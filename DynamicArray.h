...
{
private:
	T* data = nullptr; //поля класса: размер массива и указатель на элементы
	int size = 0;
public:
	DynamicArray(int);
...
template <typename T>
DynamicArray<T>::DynamicArray(T* data, int size) : DynamicArray(size) // создать массив из массиса чисел по указателю и длине
{
